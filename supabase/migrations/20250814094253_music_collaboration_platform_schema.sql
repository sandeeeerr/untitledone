...
-- RLS Policies voor projects
CREATE POLICY "Projects are viewable by everyone if public"
ON public.projects FOR SELECT
TO authenticated, anon
USING (NOT is_private);

CREATE POLICY "Users can view private projects they're members of"
ON public.projects FOR SELECT
TO authenticated
USING (
    is_private AND (
...
CREATE POLICY "Users can update projects they own or are admin of"
...
-- RLS Policies voor project_members
...
WITH CHECK (
    EXISTS (
        ...
        WHERE id = project_id AND (
            ...
            EXISTS (
                SELECT 1 FROM public.project_members 
                WHERE project_id = id AND user_id = auth.uid() AND role IN ('owner', 'admin', 'member')
            )
        )
    )
);
